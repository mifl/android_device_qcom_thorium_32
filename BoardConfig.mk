    ...
    ifeq ($(ENABLE_AB),true)
      ifeq ($(ENABLE_VENDOR_IMAGE), true)
        ifeq ($(strip $(BOARD_DYNAMIC_PARTITION_ENABLE)),true)
          TARGET_RECOVERY_FSTAB := device/qcom/msm8937_32/fstabs-4.9/recovery_AB_dynamic_variant.fstab
        else
      ...
      ifeq ($(ENABLE_VENDOR_IMAGE), true)
        ifeq ($(strip $(BOARD_DYNAMIC_PARTITION_ENABLE)),true)
          TARGET_RECOVERY_FSTAB := device/qcom/msm8937_32/fstabs-4.9/recovery_non-AB_dynamic_variant.fstab
        else
...
ifeq ($(strip $(TARGET_KERNEL_VERSION)), 4.9)
     ...
     BOARD_KERNEL_CMDLINE := console=ttyHSL0,115200,n8 androidboot.console=ttyHSL0 androidboot.hardware=qcom androidboot.memcg=false user_debug=30 msm_rtb.filter=0x237 ehci-hcd.park=3 androidboot.bootdevice=7824900.sdhci lpm_levels.sleep_disabled=1 earlycon=msm_hsl_uart,0x78B0000 loop.max_part=7 androidboot.selinux=permissive
endif
